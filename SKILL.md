...
name: github-actions
description: >
  GitHub Actions 工作流的生成、审查与优化。Generate, review, and optimize GitHub Actions workflows.
  适用场景：创建 CI/CD Pipeline、配置自动化发布、添加安全扫描、审查现有 Workflow、优化缓存与并发策略。
  Use cases: Create CI/CD pipelines, configure automated releases, add security scanning, review existing workflows, optimize caching and concurrency strategies.
  ...
  Trigger keywords: GitHub Actions, workflow, CI/CD, pipeline, YAML, actions/cache, release automation, Dependabot, concurrency, permissions, Runner, Job, Step.
---

# GitHub Actions 自动化工作流 Skill / GitHub Actions Automation Workflow Skill
