...
| Rust | 无官方 setup | 手动 `actions/cache@v4` 缓存 `target/` |
...
| 检查点 # | 位置 | 验证内容 | 验证命令 / 预期输出 |
|----------|------|----------|-------------------|
| CP1 | 骨架生成后 | `on:` 事件是否正确匹配用户需求 | 对照决策树，确认触发事件与用户请求匹配 |
| CP2 | YAML 生成后 | 是否包含三项必含配置 | 搜索文件中的 `permissions:`、`concurrency:`、`# v` (版本注释) |
| CP3 | YAML 生成后 | YAML 语法是否合法 | `yamllint .github/workflows/` 或 Python `yaml.safe_load()` |
| CP4 | 缓存配置后 | 缓存路径是否匹配语言生态 | 对照 3.4 缓存策略速查表逐项核对 |
| CP5 | Action 版本锁定 | 所有 `uses:` 是否都锁定了 SHA | 搜索 `@main`、`@master`、`@v[0-9]$` 不得出现 |
...
rg -n 'uses:\s+\S+@v\d+$' .github/workflows/          # 应返回空（必须锁 SHA）
