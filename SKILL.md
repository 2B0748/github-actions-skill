...
description: >
  ...
  Use cases: Create CI/CD pipelines, configure automated releases, add security scanning,
  review existing workflows, optimize caching and concurrency strategies.
  Trigger keywords: GitHub Actions, workflow, CI/CD, pipeline, YAML, actions/cache,
  release automation, Dependabot, concurrency, permissions, Runner, Job, Step.
---
...
| Docker | None | Use `docker/build-push-action` with `cache-from` / `cache-to` |
...
|------------|--------|-------------|
| `permissions` explicitly declared | Missing | Not set, defaults to write permission |
| Action version pinning | Missing | `actions/checkout@v4` SHA not pinned |
| `concurrency` configured | Present | Configured |
| Secret reference method | Present | Uses `${{ secrets.X }}`, no plaintext |
| `pull_request_target` risk | Present | Not used (high-risk event avoided) |
...
| Checkpoint # | Location | Verification | Verification Command / Expected Output |
|-------------|----------|-------------|----------------------------------------|
